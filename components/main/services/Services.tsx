      ...
      <Container>
        ...
        <div className="flex-row items-start py-5">
          <div className="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 gap-4">
            {servicesData.map(({ name1, name2, image, description }, index) => (
              <div
                key={index}
                className={`h-[310px] p-12 bg-zinc-100 dark:bg-background rounded-[45px] border:1px_solid_rgba(255,255,255,.1)] [box-shadow:0_-20px_80px_-20px_#8686f01f_inset] border   justify-between items-start flex text-black dark:text-white`}
              >
